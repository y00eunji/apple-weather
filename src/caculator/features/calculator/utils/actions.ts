import { HistoryType } from '@/caculator/app/context/historyContext.tsx';
import { checkIsMinusString, checkIsNull } from '@/caculator/features/calculator/utils/conditions.ts';
import { OPERATORS, OperatorsType } from '@/caculator/shared/constant/operators.ts';
...
export const actions = {
  ...
  clickOperatorBtn: ({
    ...
    setInput,
    setCurrentInput,
  }: Params & Required<Pick<Params, 'inputValue' | 'currentInput' | 'value'>>): void => {
    ...
    if (checkIsMinusString(value) && OPERATORS.includes(currentInput as OperatorsType)) {
      ...
      if (
        OPERATORS.includes(inputValue[inputValue.length - 2] as OperatorsType) &&
        checkIsMinusString(currentInput) &&
        OPERATORS.includes(value as OperatorsType)
      ) {
        return;
      } else {
        setInput(prev =>
          OPERATORS.includes(currentInput as OperatorsType) ? prev.replace(/.$/, value) : prev + value,
        );
      }
    }
  ...
  clickNumberBtn: ({
    ...
    setInput,
    setCurrentInput,
  }: Params & Required<Pick<Params, 'inputValue' | 'value'>>): void => {
    setInput(prev =>
      inputValue === '-0' || inputValue === '0'
        ? value
        : checkIsMinusString(inputValue) && value !== '0'
          ? '-' + value
          : prev + value,
    );
    setCurrentInput(value);
  ...
  clickResultBtn: ({
    ...
    setCurrentInput,
    setHistory,
  }: Params & Required<Pick<Params, 'inputValue'>>): void => {
